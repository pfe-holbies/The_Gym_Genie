...
const { prompt_map} = require('./prompts');
...
async function handleMessage(user) {
    ...
    const [err, response] = await safeAwait(openai.completions.create(parameters));
    ...
    // Return the text generated by the API
    let res = response.choices[0].text.trim();

    //let workout = '', diet = '', supplements = '';
    let [workout, diet, supplements] = res.split(',');
